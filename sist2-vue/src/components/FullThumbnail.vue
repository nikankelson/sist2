<template>
  <div
    v-if="doc._props.hasThumbnail"
    class="img-wrapper"
    @mouseenter="onTnEnter()"
    @mouseleave="onTnLeave()"
    @touchstart="onTouchStart()"
  >
    <div
      v-if="doc._props.isAudio"
      class="card-img-overlay"
      :class="{ 'small-badge': smallBadge }"
    >
      <span class="badge badge-resolution">{{
        humanTime(doc._source.duration)
      }}</span>
    </div>
    ...
    <div
      v-if="
        (doc._props.isVideo || doc._props.isGif) && doc._source.duration > 0
      "
      class="card-img-overlay"
      :class="{ 'small-badge': smallBadge }"
    >
    ...
    <img
      ...
      alt=""
      :style="{
        height: doc._props.isGif && hover ? `${tnHeight()}px` : undefined,
      }"
      class="pointer fit card-img-top"
      @click="onThumbnailClick()"
    />
    <img v-else :src="tnSrc" alt="" class="fit card-img-top" />

    <ThumbnailProgressBar
      v-if="hover && doc._props.hasVidPreview"
...
export default {
  ...
  created() {
    this.$store.subscribe((mutation) => {
      if (
        mutation.type === "busTnTouchStart" &&
        mutation.payload !== this.doc._id
      ) {
        this.onTnLeave();
      }
    });
  ...
  methods: {
    ...
    scheduleNextTnNum(offset = 0) {
      const INTERVAL =
        (this.$store.state.optVidPreviewInterval ?? 700) - offset;
